...
<header class="kcs-header">
    ...
    <?php if ($loggedIn): ?>
        <div class="kcs-header__user">
            <button class="kcs-header__member-btn" id="btnOpenMember">
                <span class="kcs-header__avatar"><?= htmlspecialchars($_initials) ?></span>
                <?= htmlspecialchars($memberName ?? '') ?>
            </button>
            <a href="<?= APP_BASE_PATH ?>/logout.php" class="kcs-btn kcs-btn--ghost kcs-btn--sm">Abmelden</a>
        </div>
    <?php else: ?>
        <button class="kcs-btn kcs-btn--primary kcs-btn--sm" data-bs-toggle="modal" data-bs-target="#loginModal">
